...
do
    ...
    if [ $stage -le 1 ]; then
        cd /opt/AusKidTalkv2/
        ! $PYTHON tools/Initiate_Alignment/InitAlign.py $childID "$LOCAL_OUT_DIR/primary_16b.wav" $LOCAL_OUT_DIR/txtgrids && mv InitAlign.log "$LOCAL_OUT_DIR" && echo 100 > $LOCAL_OUT_DIR/stage && continue
